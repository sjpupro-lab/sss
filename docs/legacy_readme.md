# SSS — legacy README (Phase 1 → Phase 4 era)

> **This document is archived.** The post-Phase-5 entry point is the
> repo-root `README.md`. The text below is the README as it stood at
> the start of Phase 5 — preserved for the verified benchmarks, the
> Phase 1–4 narrative, and the historical CE-Cell + spatial-pattern
> documentation that the new README references at a higher level.
> Anything that mentions `gen_image_ce`, `sss_animate`, or the `.ces`
> format applies to the pre-Phase-5 codebase only.
